...
export const AlertPanel: React.FC<AlertPanelProps> = ({ roomId }) => {
  ...
  // 2. Memoize filtered list to show active un-resolved alerts
  const roomAlerts = useMemo(() => {
    return (alerts || []).filter(
      (alert) => alert.roomId === roomId && alert.stage !== 'resolved'
    );
  }, [alerts, roomId]);
  ...
  return (
    <div className="space-y-3">
      {roomAlerts.map((alert) => {
        const isCritical = alert.stage === 'confirmed' || alert.stage === 'escalated';
        const title = alert.stage === 'verifying' 
          ? 'Possible Fall (Verifying)' 
          : alert.stage === 'confirmed'
          ? 'Confirmed Fall (Critical)'
          : alert.stage === 'acknowledged'
          ? 'Fall Alert Acknowledged'
          : alert.stage === 'escalated'
          ? 'Fall Alert Escalated'
          : 'Fall Alert';
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Confidence: {alert.confidence}% • {new Date(alert.createdAt).toLocaleTimeString()}
                </p>
              </div>
            </div>

            {alert.stage !== 'acknowledged' && alert.stage !== 'escalated' && (
              <button
                onClick={() => acknowledgeAlert(alert.id, session?.displayName ?? 'Staff Member')}
